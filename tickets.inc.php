 ...
 * 2) Fill out the Oject Type (which can be found via the database in the ost_form and ost_form_entry table.)
 * 3) Fill out the field ID (which can be found via the database in the ost_form_entry_values table.)
 * 4) Fill out the heading (Enter the heading for your custom column item.)
 * 5) Backup your current file /include/staff/tickets.inc.php
 * 6) Upload your new tickets.inc.php to /include/staff/tickets.inc.php 
 */
$custom_item['object_type'] = "U"; //TYPE from ost_form and ost_form_entry tables. (Currently you have to manually add this value to line 282 below)
$custom_item['field_id'] = "34"; //ID from from ost_form_entry_values table.
$custom_item['heading'] = "Company"; //Column heading displayed to front end users.
...
//Queues: Overloaded sub-statuses
if($staffId && ($staffId==$thisstaff->getId())) { //My tickets
    ...
    /* Showing assigned tickets on open queue?
       Don't confuse it with show assigned To column -> 
     */
...
if($search):
    ...
    if($searchTerm){
        ...
        if (is_numeric($searchTerm)) {
            ...
            //..mass scan! search anything that moves!
...
$qfrom.=' LEFT JOIN '.TICKET_LOCK_TABLE.' tlock ON (ticket.ticket_id=tlock.ticket_id AND tlock.expire>NOW()
       ...
       .' LEFT JOIN '.TABLE_PREFIX.'form_entry fentry ON (user.id = fentry.object_id) AND fentry.object_type = "U" '  // object id and type $custom_item['object_type'] = "U";
       .' LEFT JOIN '.TABLE_PREFIX.'form_entry_values fentry_val ON (fentry.id = fentry_val.entry_id) AND fentry_val.field_id = '.$custom_item["field_id"] // link to the form values
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="940">
    <thead>
        <tr>
            ...
            title="Sort By <?php echo $custom_item['heading']; ?> <?php echo $negorder; ?>"><?php echo $custom_item['heading']; ?></a></th>
            <!-- AB – Custom Col 1 -->
        </tr>
     </thead>
     <tbody>
